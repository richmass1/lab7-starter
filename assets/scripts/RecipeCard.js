...
class RecipeCard extends HTMLElement {
	...
	constructor() {
		...
		const shdwElt = this.attachShadow({ mode: 'open'});
		// A2. TODO - Create an <article> element - This will hold our markup once our data is set
		const artElt = document.createElement('article');
		...
		shdwElt.append(myStyles);
		shdwElt.append(artElt);
	}
	...
	set data(data) {
		...
		const artElt = this.shadowRoot.querySelector('article');
		...
		recipeLink.innerText = data.titleTxt;
		recipeLink.href = data.titleLnk;
		para1.append(recipeLink);

		const para2 = document.createElement('p');
		para2.className = 'organization';
		para2.innerText = data.organization;
		...
		ratingSpan2.innerText = data.numRatings;
		ratingDiv.append(ratingSpan, ratingImg, ratingSpan2);

		const timeReq = document.createElement('time');
		timeReq.innerText = data.lengthTime;

		const ingredients = document.createElement('p');
		ingredients.className = 'ingredients';
		ingredients.innerText = data.ingredients;

		artElt.append(image, para1, para2, ratingDiv, timeReq, ingredients);
	}
}
